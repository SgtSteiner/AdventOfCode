...
with a password based on the previous one. Corporate policy dictates that passwords must be exactly
eight lowercase letters (for security reasons), so he finds his new password by incrementing his
old password string repeatedly until it is valid.

Incrementing is just like counting with numbers: xx, xy, xz, ya, yb, and so on.
Increase the rightmost letter one step; if it was z, it wraps around to a, and repeat with the next letter
...
print("".join(password))
for c in range(len(password)-1, -1, -1):
    char = password[c]
    while char != "z":
        password[c] = chr(ord(char)+1)
        char = chr(ord(char)+1)
        print("".join(password))
