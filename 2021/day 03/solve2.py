...
def verifica_boards(first_board=True, num=0):
    ...
    for board_ in range(len(boards)):
        ...
        for line in boards[board_]:
            ...
            if len(premiados) == 5 and first_board:
                line_bingo = True
                n_board = board_
            elif len(premiados) == 5 and num in premiados:
                line_bingo = True
                n_board = board_
...
if __name__ == "__main__":
    ...
    print(f"Part two: {sum_unmarked_board(last_board_win) * last_number}")
