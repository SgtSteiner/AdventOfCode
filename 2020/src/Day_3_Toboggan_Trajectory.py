"""
Enunciado del problema: https://adventofcode.com/2020/day/3

With the toboggan login problems resolved, you set off toward 
the airport. While travel by toboggan might be easy, it's certainly 
...
# Count the number of trees in the pattern (puzzle map)
def trees_count(puzzle_map, slope):
    step_right, step_down = slope
    row, col = step_down, step_right
    res_count = 0

    while row in range(len(data)):
...
total_trees = [trees_count(data, slope) for slope in slopes]
print(f"Resultado parte 2: {np.prod(total_trees)}")
