"""
Enunciado del problema: https://adventofcode.com/2020/day/2
...
it contains no instances of b, but needs at least 1. The first and 
third passwords are valid: they contain one a or nine c, both within 
...
for element in data:
    min_length, max_length = map(int, element[0].split("-")) # lowest and highest number of times
    letter = element[1][0]
    password = element[2]

    # Validate policy rules # 1
    if min_length <= password.count(letter) <= max_length:
        result_part1 += 1

    # Validate policy rules # 2
    if password[min_length-1] != password[max_length-1]:
        if password[min_length-1] == letter or password[max_length-1] == letter:
            result_part2 += 1
            
print(f"Resultado parte 1: {result_part1}\nResultado parte 2: {result_part2}")